...
export class ChartJsHelper {
     ...
     */
    public static buildChartData(stats: ChartStatistics, cumulative: boolean = false): ChartData<'line'> {
        ...
        for (const config of datasetConfigs) {
            const rawData = stats[config.key as keyof ChartStatistics] as TimeSeriesData[] || [];
            const data = cumulative ? this.calculateCumulative(rawData) : rawData.map(item => item.count);
            datasets.push({
   ...
   */
    ...
    public static getColors(theme: ThemeType): ChartColors {
        ...
        return {
            ...
            tooltipBackground: isDark ? 'rgba(33, 37, 41, 0.95)' : 'rgba(255, 255, 255, 0.95)',
