...
export class ContentTypeHelper {
  ...
  private static readonly CATEGORIES: Record<string, CategoryConfig> = {
    ...
    'video': {
      ...
      extensions: ['mp4', 'avi', 'mov', 'wmv', 'flv', 'webm', 'mkv', '3gp', 'ogv'],
      icon: 'videocam',
    ...
    'document': {
      patterns: [
        ...
        { type: ContentTypeHelper.MATCH_TYPE.EQUALS, value: 'application/msword' },
        { type: ContentTypeHelper.MATCH_TYPE.EQUALS, value: 'application/vnd.openxmlformats-officedocument.wordprocessingml.document' },
        { type: ContentTypeHelper.MATCH_TYPE.INCLUDES, value: 'excel' },
        { type: ContentTypeHelper.MATCH_TYPE.EQUALS, value: 'application/vnd.ms-excel' },
        { type: ContentTypeHelper.MATCH_TYPE.EQUALS, value: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' },
        { type: ContentTypeHelper.MATCH_TYPE.INCLUDES, value: 'powerpoint' },
        { type: ContentTypeHelper.MATCH_TYPE.EQUALS, value: 'application/vnd.ms-powerpoint' },
        { type: ContentTypeHelper.MATCH_TYPE.EQUALS, value: 'application/vnd.openxmlformats-officedocument.presentationml.presentation' }
      ],
    ...
    'other': {
      patterns: [
        ...
        { type: ContentTypeHelper.MATCH_TYPE.EQUALS, value: 'application/x-rar-compressed' },
        { type: ContentTypeHelper.MATCH_TYPE.EQUALS, value: 'application/x-7z-compressed' },
        { type: ContentTypeHelper.MATCH_TYPE.EQUALS, value: 'application/x-tar' },
        { type: ContentTypeHelper.MATCH_TYPE.EQUALS, value: 'application/gzip' }
      ],
   ...
   */
  static getFilterCategories(): { key: string, icon: string, translationKey: string }[] {
   ...
   */
  static validateFileType(file: File, allowedExtensions?: string[]): {
    isValid: boolean;
    category: string;
    reason?: string;
  } {
    ...
    if (mimeType && !this.isMimeTypeConsistentWithExtension(mimeType, fileExtension)) {
      return {
        isValid: false,
        category: categoryKey,
   ...
   */
  private static isMimeTypeConsistentWithExtension(mimeType: string, extension: string): boolean {
    ...
    const extensionCategory = Object.entries(this.CATEGORIES).find(([_, config]) =>
      config.extensions.map(ext => ext.toLowerCase()).includes(extension.toLowerCase())
    )?.[0] || 'other';

    // They should match the same category
    return mimeCategory === extensionCategory || mimeCategory === 'other' || extensionCategory === 'other';
  }
   ...
   */
  private static matchesPattern(contentType: string, pattern: { type: string, value: string }): boolean {
    switch (pattern.type) {
   ...
   */
  static getIcon(item: ILibraryItem | IItem): string {
    ...
    if (fileExtension) {
      ...
      for (const categoryConfig of Object.values(this.CATEGORIES)) {
        if (categoryConfig.specificIcons && categoryConfig.specificIcons[fileExtension]) {
      ...
      for (const [_, categoryConfig] of Object.entries(this.CATEGORIES)) {
        if (categoryConfig.extensions.map(ext => ext.toLowerCase()).includes(fileExtension)) {
  ...
  * Get category of file based on content type
   ...
   */
  static getCategory(item: ILibraryItem | IItem): string {
    if (!item.contentType) {
      return 'other';
    }
