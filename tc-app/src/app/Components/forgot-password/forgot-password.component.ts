  ...
  submit() {
    ...
    this.authService.requestPasswordReset(email).subscribe({
      next: () => {
        this.loading = false;
        if (this.isLoggedIn) {
          this.snackbarService.success('Password change email sent. Please check your inbox to continue.');
          this.router.navigate(['/dashboard']);
        } else {
          this.snackbarService.success('Password reset email sent. Please check your inbox.');
