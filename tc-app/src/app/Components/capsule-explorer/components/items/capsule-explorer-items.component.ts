import { CommonModule } from '@angular/common';
import { Component, Input } from '@angular/core';
import { MatCardModule } from '@angular/material/card';
import { MatIconModule } from '@angular/material/icon';
import { TranslocoModule } from '@jsverse/transloco';
import { IItem, ICapsule } from '../../../../Models/models.interface';
import { DateFormatPipe } from '../../../../Pipes/date-format.pipe';
import { ContentTypeHelper } from '../../../../Utils/content-type.helper';
import { ContentViewerService } from '../../../../Features/content-viewer/content-viewer.service';

@Component({
  selector: 'tc-capsule-explorer-items',
  standalone: true,
  imports: [
    CommonModule,
    TranslocoModule,
    MatCardModule,
    MatIconModule,
    DateFormatPipe
  ],
  templateUrl: './capsule-explorer-items.component.html',
  styleUrl: './capsule-explorer-items.component.scss'
})
export class CapsuleExplorerItemsComponent {

  @Input() items: IItem[] = [];
  @Input() capsule?: ICapsule;

  constructor(private contentViewerService: ContentViewerService) { }

  getIcon(item: IItem): string {
    return ContentTypeHelper.getIcon(item);
  }

  viewContent(item: IItem) {
    this.contentViewerService.showContent(item);
  }

  isTimeLocked(): boolean {
    if (!this.capsule || !this.capsule.openDate) {
      return false;
    }
    
    const openDate = new Date(this.capsule.openDate);
    const now = new Date();
    return openDate > now;
  }

  getItemsCount(): number {
    return this.capsule?.usage?.items_per_capsule  || 0;
  }

  getFormattedOpenDate(): string {
    if (!this.capsule?.openDate) {
      return '';
    }
    
    const openDate = new Date(this.capsule.openDate);
    return openDate.toLocaleDateString();
  }

  getFakeItems(): Array<{ svg: string, title: string, date: string }> {
    const fakeItems = [
      {
        svg: `<svg viewBox="0 0 24 24" fill="currentColor">
                <path d="M14,2H6A2,2 0 0,0 4,4V20A2,2 0 0,0 6,22H18A2,2 0 0,0 20,20V8L14,2M18,20H6V4H13V9H18V20Z"/>
              </svg>`,
        title: 'Secret Document',
        date: 'Hidden until opening'
      },
      {
        svg: `<svg viewBox="0 0 24 24" fill="currentColor">
                <path d="M21,16.5C21,16.88 20.79,17.21 20.47,17.38L12.57,21.82C12.41,21.94 12.21,22 12,22C11.79,22 11.59,21.94 11.43,21.82L3.53,17.38C3.21,17.21 3,16.88 3,16.5V7.5C3,7.12 3.21,6.79 3.53,6.62L11.43,2.18C11.59,2.06 11.79,2 12,2C12.21,2 12.41,2.06 12.57,2.18L20.47,6.62C20.79,6.79 21,7.12 21,7.5V16.5M12,4.15L6.04,7.5L12,10.85L17.96,7.5L12,4.15Z"/>
              </svg>`,
        title: 'Mystery Package',
        date: 'Waiting to be unveiled'
      },
      {
        svg: `<svg viewBox="0 0 24 24" fill="currentColor">
                <path d="M12,2A10,10 0 0,0 2,12A10,10 0 0,0 12,22A10,10 0 0,0 22,12A10,10 0 0,0 12,2M7.07,18.28C7.5,17.38 10.12,16.5 12,16.5C13.88,16.5 16.5,17.38 16.93,18.28C15.57,19.36 13.86,20 12,20C10.14,20 8.43,19.36 7.07,18.28M18.36,16.83C16.93,15.09 13.46,14.5 12,14.5C10.54,14.5 7.07,15.09 5.64,16.83C4.62,15.5 4,13.82 4,12C4,7.59 7.59,4 12,4C16.41,4 20,7.59 20,12C20,13.82 19.38,15.5 18.36,16.83M12,6C10.06,6 8.5,7.56 8.5,9.5C8.5,11.44 10.06,13 12,13C13.94,13 15.5,11.44 15.5,9.5C15.5,7.56 13.94,6 12,6M12,11A1.5,1.5 0 0,1 10.5,9.5A1.5,1.5 0 0,1 12,8A1.5,1.5 0 0,1 13.5,9.5A1.5,1.5 0 0,1 12,11Z"/>
              </svg>`,
        title: 'Personal Memory',
        date: 'Soon to be revealed'
      }
    ];

    // Return only the number of items that actually exist, max 3
    const itemCount = Math.min(this.getItemsCount(), 3);
    return fakeItems.slice(0, itemCount);
  }
}