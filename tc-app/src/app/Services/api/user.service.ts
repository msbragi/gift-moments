...
export class UserService {
   ...
   */
  updateCurrentUser(userData: { email: string; fullName?: string; avatar?: string }): Observable<IUser> {
    return this.apiService.put<IResponse>('users/me', userData, 'Profile updated successfully').pipe(
      map((response: IResponse) => {
        return response.data as IUser;
      })
    );
