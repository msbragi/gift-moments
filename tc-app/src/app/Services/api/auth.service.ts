...
import { IAuthResponse, IRefreshTokenResponse, IResponse } from '../../Models/auth.interface';
...
export class AuthService {
  ...
  private checkAuthStatus(): void {
    ...
    if (token) {
      ...
      this.apiService.get<IResponse>('users/me').pipe(
        map((response: IResponse) => response.data as IUser)
      ).subscribe({
        next: (user) => {
  ...
  login(credentials: IAuth): Observable<IAuthResponse> {
    return this.apiService.post<IAuthResponse>('auth/login', credentials, 'Login successful')
      .pipe(
  ...
  googleLogin(token: string): Observable<IAuthResponse> {
    ...
    return this.apiService.post<IAuthResponse>('auth/google', request, 'Google login successful')
      .pipe(
        tap((response: IAuthResponse) => {
  ...
  refreshToken(): Observable<any> {
    ...
    return this.apiService.post<IRefreshTokenResponse>(
      'auth/refresh-token',
   ...
   */
  register(userData: { email: string; password: string }): Observable<IResponse> {
 ...
 */
  ...
  private fetchCurrentUserData(): void {
    ...
    this.apiService.get<IResponse>('users/me').pipe(
      ...
      error: (error) => {
        console.error('AuthService: Failed to load complete user data:', error);
        this.logout();
      }
    });
  }
