...
import { FileUploadConfig, FileUploadEvent } from '../../Models/upload.model';
...
export class DropUploadComponent implements OnInit {
  ...
  onFileDropped(event: DragEvent): void {
    ...
    if (this.config.allowMultiple) {
      console.warn('Multiple file support is not fully implemented yet, using only first file');
    }
  ...
  validateFile(file: File): boolean {
    ...
    if (this.allowedExtensions.length > 0) {
      ...
      if (file.type && !this.isMimeTypeConsistentWithExtension(file.type.toLowerCase(), fileExtension)) {
   ...
   */
  private isMimeTypeConsistentWithExtension(mimeType: string, extension: string): boolean {
    // Basic checks for common inconsistencies
    if (extension.match(/^(jpg|jpeg|png|gif|webp|svg|bmp|ico|tiff|tif)$/) && !mimeType.startsWith('image/')) {
      return false;
    }
    if (extension.match(/^(mp4|avi|mov|wmv|flv|webm|mkv|3gp|ogv)$/) && !mimeType.startsWith('video/')) {
      return false;
    }
    if (extension.match(/^(mp3|wav|flac|aac|ogg|wma|m4a|opus)$/) && !mimeType.startsWith('audio/')) {
      return false;
    }
  ...
  getFileIcon(file: File): string {
    return ContentTypeHelper.getIcon({ contentType: file.type} as IItem);
  }
