...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, MatIconModule, MatButtonModule, MatTooltipModule, TranslocoModule],
  template: `
  <span *transloco="let t;">
    <button *ngIf="isExpiring"
            mat-icon-button 
            color="accent"
...
export class TokenStatusComponent implements OnInit, OnDestroy {
  ...
  private checkTokenStatus(): void {
    ...
    if (this.authService.isTokenExpiringSoon(5)) { // 5 minute warning
