    ...
    private loadDefaultFooter(): void {
        ...
        const defaultSub = this.http.get(url, { responseType: 'text' }).subscribe({
            ...
            error: (error) => {
                ...
                this.footerDefault = this.sanitizer.bypassSecurityTrustHtml(`<p>Footer content not available</p>`);
                this.footer = this.footerDefault;
                this.isDefaultLoaded = true;
            }
        });
...
private loadFooterForLanguage(lang: string): void {
    ...
    const footerSub = this.http.get(`assets/i18n/footer/${footerLang}.html`, { responseType: 'text' })
        .pipe(
            map(content => this.sanitizer.bypassSecurityTrustHtml(content)),
            catchError(error => {
                console.error(`Failed to load footer for language: ${footerLang}`, error);
