...
const shouldExcludeRequest = (url: string): boolean => {
  ...
  const staticFileExtensions = ['.json', '.png', '.jpg', '.jpeg', '.gif', '.svg', '.ico', '.css', '.js'];
  ...
  return next(request).pipe(
    catchError((error: HttpErrorResponse) => {
      ...
      if (error.status === 401) {
        ...
        return httpClient.post<IRefreshTokenResponse>(
          ...
          switchMap((response: IRefreshTokenResponse) => {
            ...
            return throwError(() => new Error('Token refresh failed - no access_token'));
          }),
          catchError(refreshError => {
            // If refresh fails, clear auth state and redirect to login
            clearAuthState();

            snackBar.open('Your session has expired. Please log in again.', 'OK', {
              duration: 15000
            });
